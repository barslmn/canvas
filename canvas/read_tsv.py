...
def read_sample_from_tsv(file_path):
    ...
    try:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for row_number, row in enumerate(reader, start=2):  # Start at 2 considering header row
                ...
                sample_type = row.get("sample_type")  # May be None if 'sample_type' column is missing

                # Validate 'inst' field
                if inst:
                    # Attempt to find a unique Institution matching the first 5 characters (case-insensitive)
                    inst_validated_qs = Institution.objects.filter(name__icontains=inst[:5])
                ...
                row_dict = {
                    ...
                    "sample_type": sample_type.strip() if isinstance(sample_type, str) else sample_type
                }
        ...
        raise UnicodeDecodeError("The file encoding is not UTF-8. Please ensure the TSV file is UTF-8 encoded.")
